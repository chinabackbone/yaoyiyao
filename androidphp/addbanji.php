...
$sql = "SELECT * from banji_info where tusername='$tusername' and banji = '$banji' and type='t'";
...
if ($result = $mydb->query($sql)) {
    ...
    if ($row[0] != null) {
        ...
        try {
            $sql = "insert into banji_info (tusername,banji,type) values ('$tusername','$banji','t')";
            if ($mydb->query($sql)) {
                //成功
                $user->ok = 1;
            } else {
